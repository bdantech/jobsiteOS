-- O Explorador quebrava (27s/página) porque a view era UNION ALL de duas fontes: o
-- universo de construção (876k) e as empresas de lista FORA do universo (2.250,
-- não-construção, sem CNAE). A UNION forçava materializar tudo antes de ordenar. Sozinha,
-- a parte 1 (universo) pagina em ~53ms. As 2.250 seguem no módulo Empresas.
-- (Este passo usou o cast capital_social::numeric para o create-or-replace casar o tipo
--  da coluna vindo da UNION; a 0022 remove o cast, que atrapalhava a ordenação.)
...
with (security_invoker = true) as
  select
    ...
    u.porte_rfb, u.cnae_principal, u.cnaes_todos, u.cnae_grupos,
    u.capital_social::numeric as capital_social,
