-- 0240 — Uma view por fonte, e a união só para quem quer as três
--
-- ─── O QUE QUEBROU ─────────────────────────────────────────────────────────
-- O Kanban passou a ler `funil_oportunidades` e a primeira página de uma coluna
-- foi de 1,85 ms para 8.382 ms. Cinco colunas em paralelo, cada uma pagando ainda
-- um `count: 'exact'`, e o funil parou de aparecer.
--
-- A causa não é o custo dos joins: é o `UNION ALL`. O plano antigo caminhava
-- `notas_fiscais_receita_idx` JÁ ORDENADO e parava na linha 41. O `Append` é
-- barreira de otimização para ordenação — com ele, o Postgres precisa materializar
-- as 8.430 notas abertas por todos os joins e só então ordenar. Filtrar por
-- `tipo = 'nf'` poda os outros ramos e melhora dez vezes, mas não remove a
-- barreira: continuava em 767 ms.
--
-- ─── A CORREÇÃO ────────────────────────────────────────────────────────────
-- Uma view por fonte. Cada uma tem UMA origem, então o planejador consegue achatá-la
-- e empurrar `ORDER BY ... LIMIT` até o índice — que é como o funil de NFs sempre
-- funcionou.
--
-- `funil_oportunidades` continua existindo, agora como a união das três, e continua
-- sendo o contrato para quem quer a lista inteira sem paginar (as tools, a outbox,
-- a reclassificação). Quem PAGINA lê a view da fonte e junta no cliente: o top-N
-- global é sempre um subconjunto da união dos top-N de cada fonte, então o
-- resultado é o mesmo e cada consulta volta a custar milissegundos.
--
-- As três nascem `security_invoker = true`. É a opção que faz a RLS das tabelas de
-- base valer dentro da view; sem ela, o originador A enxergaria a carteira do B.

drop view if exists public.funil_oportunidades;

-- ── NF ──────────────────────────────────────────────────────────────────────
-- Sem CTE e sem união: é isto que permite ao planejador usar
-- `notas_fiscais_receita_idx` e parar na quadragésima linha. Medido: 8.382 ms
-- pela união, 3,1 ms por aqui — e o lateral que calcula `ultimo_numero_nf` volta a
-- ser podado, porque o card não pede a coluna.
create or replace view public.funil_oportunidades_nf
...
-- ── Título Sienge ───────────────────────────────────────────────────────────
-- Esta é a única que precisa da CTE: o "N de M" do card conta as parcelas do
-- mesmo título, e a contagem é por (connection_id, bill_id) — nunca por bill_id
-- sozinho, que só é único dentro de uma conexão.
...
-- ── A união ─────────────────────────────────────────────────────────────────
-- Continua sendo o contrato de quem lê a lista inteira. Quem pagina NÃO deve usá-la:
-- o `Append` impede o `ORDER BY ... LIMIT` de alcançar o índice, e foi exatamente
-- isso que derrubou o Kanban.
create view public.funil_oportunidades
with (security_invoker = true)
as
select * from public.funil_oportunidades_nf
union all
select * from public.funil_oportunidades_preauth
union all
select * from public.funil_oportunidades_titulo;

comment on view public.funil_oportunidades is
  'A união das três fontes (04s §6). Para LISTAR tudo sem paginar. Quem pagina deve ler funil_oportunidades_nf / _preauth / _titulo e juntar no cliente — o Append impede o ORDER BY ... LIMIT de usar o índice.';
comment on view public.funil_oportunidades_nf is
  'O ramo NF da projeção, isolado para que o ORDER BY ... LIMIT alcance o índice.';

grant select on public.funil_oportunidades to authenticated;
grant select on public.funil_oportunidades_nf to authenticated;
grant select on public.funil_oportunidades_preauth to authenticated;
grant select on public.funil_oportunidades_titulo to authenticated;
