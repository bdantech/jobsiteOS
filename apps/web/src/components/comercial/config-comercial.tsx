...
export function ConfigComercial() {
  ...
  const [aba, setAba] = React.useState('time')
  ...
  return (
    <div className="space-y-4">
      ...
      {/*
        ─── POR QUE ABAS, E POR QUE ESTAS CINCO ────────────────────────────────
        A tela era uma coluna só com oito seções, e a última — a régua do Meu Dia, com um
        seletor de cargo e treze blocos — ficava a quatro rolagens do topo. Numa tela de
        configuração isso não é só incômodo: o que está longe é o que ninguém sabe que
        existe, e um parâmetro que ninguém sabe que existe fica no default para sempre.

        A divisão é por PERGUNTA, não por tamanho. Quem chega aqui já sabe o que veio
        mexer, e a aba é o que responde "onde fica isso":

          Time         quem vende, e quem está olhando para quem
          Funil        como o trabalho entra e como ele morre
          Comissão     tudo o que vira dinheiro
          Fornecedores o funil de cadastro e o custo do clique
          Meu Dia      o que cada cargo vê quando abre o dia

        Três coisas MUDARAM DE LUGAR no caminho, e não por arrumação: o card "Painel e
        passivos" juntava leaderboard (que é sobre o time), mínimo de antecipações (que
        classifica a conta e por isso decide a TAXA) e estorno de no-show (que é uma
        regra de comissão dentro de um card que não fala de comissão). Um título com "e"
        no meio costuma ser duas coisas, e era.
      */}
      <Tabs value={aba} onValueChange={setAba} className="space-y-4">
        ...
        <TabsContent value="time" className="mt-0 space-y-4">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">Painel do time</CardTitle>
              <CardDescription>
                O que a equipe vê sobre si mesma, e a partir de quando o silêncio de alguém
                vira alerta.
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="comissao" className="mt-0 space-y-4">
          ...
          */}
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">Classificação da conta</CardTitle>
              <CardDescription>
                Estes dois números decidem quando uma conta passa a contar como{' '}
                <strong>passiva</strong> — e a natureza da conta é o que escolhe a taxa entre
                `vend_passivo_*` e `vend_prospeccao_ativa_*` lá em cima. Por isso eles moram
                aqui, e não junto do painel: eles não mudam o que se vê, mudam o que se paga.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <dl className="space-y-1.5 text-sm">
        ...
        <TabsContent value="fornecedores" className="mt-0 space-y-4">
          {/*
            As CREDENCIAIS não estão aqui: usuário, senha e cliente da Nova Vida e a chave
            do Google Places vivem só em variável de ambiente do worker.
            `fornecedores_config` é lida por `authenticated` para que o card mostre o custo
            do clique — pôr uma credencial nela seria distribuí-la a todo mundo que tem o
            módulo.
          */}
          <ConfigFornecedores />
        </TabsContent>
        ...
        {/* ── Meu Dia ──────────────────────────────────────────────────────── */}
        <TabsContent value="meu-dia" className="mt-0 space-y-4">
          {/*
            A única aba que não configura o SISTEMA — configura a LEITURA que cada cargo
            faz dele. Ela lê o catálogo de blocos de packages/core, então um bloco novo
            nasce configurável aqui sem ninguém tocar nesta tela.
          */}
          <ConfigMeuDia />
        </TabsContent>
      </Tabs>

      {/* O diálogo fica FORA das abas: dentro, trocar de aba com ele aberto o
